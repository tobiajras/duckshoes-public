...
function ProductDetails() {
  ...
  const handleButton = (productToCart, color, size) => {
    if (!cart.find((cartItem) => cartItem.id === productToCart.id)) {
      cartAdd({
        id: productToCart.id,
        grid_picture_url: productToCart.grid_picture_url,
        name: productToCart.name,
        retail_price_cents: productToCart.retail_price_cents,
        color: color,
        size: size,
        quantity: 1,
      });
                        ...
                        <button
                          ...
                          onClick={() => {
                            addedToCart(productFound.id),
                              handleButton(productFound, colorParam, sizeParam);
                          }}
                        >
                          Add to cart
                        </button>
